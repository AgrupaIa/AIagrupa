...
document.addEventListener('DOMContentLoaded', function() {
    ...
    return max === min ? data.map(() => 0.5) : data.map(v => (v - min) / (max - min));
}
...
async function applyKMeans(data, clusters = 3, iterations = 10) {
    try {
        ...
        // Convertir a tensor 2D y normalizar
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            const distances = tf.sqrt(
                tf.sum(
                    tf.square(
                        tf.sub(tensorData.expandDims(1), currentCentroids.expandDims(0))
                    ),
                    2
                )
            );
            ...
            for (let c = 0; c < clusters; c++) {
                const mask = tf.equal(assignments, c);
                const filteredData = tf.booleanMask(tensorData, mask);
                const newCentroid = filteredData.size > 0 ? 
                    tf.mean(filteredData, 0) : 
                    tf.slice(currentCentroids, [c, 0], [1, 1]);
                newCentroids.push(newCentroid);
            }
            currentCentroids.dispose();
            currentCentroids = tf.concat(newCentroids, 0);
        }

        // Obtener asignaciones finales
        const finalAssignments = await tf.argMin(
            tf.sqrt(
                tf.sum(
                    tf.square(
                        tf.sub(tensorData.expandDims(1), currentCentroids.expandDims(0))
...
document.getElementById('clusterBtn').addEventListener('click', async () => {
    if (!processedData || processedData.length === 0) {
        alert('Primero carga un archivo Excel con datos');
        return;
    }
    ...
    try {
        ...
        if (numericColumns.length === 0) {
            alert('No se encontraron columnas numéricas para analizar');
            return;
        }

        // Priorizar columnas comunes como 'ventas' o 'total'
        const defaultColumn = numericColumns.includes('ventas') ? 'ventas' : 
                           numericColumns.includes('total') ? 'total' : 
                           numericColumns[0];

        const columnToAnalyze = prompt(
            `Ingrese la columna numérica a analizar (Disponibles: ${numericColumns.join(', ')}):`,
            defaultColumn
        );

        if (!columnToAnalyze || !numericColumns.includes(columnToAnalyze)) {
            alert(`Debes seleccionar una de estas columnas: ${numericColumns.join(', ')}`);
            return;
        }

        // Extraer y limpiar datos
        const rawData = processedData
            .map(row => parseFloat(row[columnToAnalyze]))
            .filter(v => !isNaN(v));

        if (rawData.length === 0) {
            alert(`La columna "${columnToAnalyze}" no contiene valores numéricos válidos`);
            return;
        }

        // Mostrar estado de procesamiento
        const analysisContent = document.getElementById('analysisContent');
        analysisContent.innerHTML = '<p>Procesando datos con IA K-Means...</p>';

        // Normalizar datos
        const normalizedData = normalizeData(rawData);

        // Ejecutar K-Means
        const groups = await applyKMeans(normalizedData, 3);

        // Añadir grupos al dataset
        processedData.forEach((row, index) => {
            row.Grupo = groups[index];
        });
        ...
        let analysisHTML = `
            <h3>Análisis de Clustering (K-Means)</h3>
            <p>Columna analizada: <strong>${columnToAnalyze}</strong></p>
            <p>Total de registros: <strong>${rawData.length}</strong></p>
            <h4>Distribución de grupos:</h4>
            <ul>
                ${Object.entries(groupCounts).map(([group, count]) => `
                    <li>${group}: ${count} clientes (${((count/rawData.length)*100).toFixed(1)}%)</li>
                `).join('')}
            </ul>
            <h4>Interpretación:</h4>
            <p>Los clientes se han clasificado en 3 grupos según sus ${columnToAnalyze}:</p>
            <ol>
                <li><strong>Grupo 1</strong>: Valores más bajos</li>
                <li><strong>Grupo 2</strong>: Valores intermedios</li>
                <li><strong>Grupo 3</strong>: Valores más altos</li>
            </ol>
        `;
        ...
        document.getElementById('analysisContent').innerHTML = 
            `<p class="error">Error en el análisis: ${error.message}</p>`;
    }
});
});
